...
SSD1306Wire display(OLED_ADDRESS, PIN_SDA, PIN_SCL); 
OLEDDisplayUi ui     ( &display );
int frameCount = 5;
int nrShotsTemp = 0;
int displaySpeed = 80;
FireMode displayMode = Single;
String fireModeStr[5] {"[ None ]","[ Single ]","[Double]", "[ Triple ]","[  Auto  ]"};
...
void setFireMode(bool state){
  if(state == true){
    ui.switchToFrame(4);
    ui.setTargetFPS(1);
    if(DEBUG) Serial.println("firemode active (OLEDFPS=0)");
  }
  else{
    ui.switchToFrame(0);
    ui.setTargetFPS(OLED_FPS);
    if(DEBUG) Serial.println("firemode inactive");
  }
}
